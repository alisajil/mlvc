...
class Scorer(nn.Module):
    def __init__(self, pretrained: str, device: str):
        super().__init__()
        tokenizer, model, image_processor = load_pretrained_model(pretrained)
        model.to(device)
        prompt = "USER: How would you rate the quality of this image?\n<|image|>\nASSISTANT:" \
                 " The quality of the image is"

        self.preferential_ids_ = [id_[1]
                                  for id_ in tokenizer(["excellent", "good", "fair", "poor", "bad"])["input_ids"]]
    ...
    def forward(self, image: List[Image.Image]):
        ...
        with torch.inference_mode():
            image_tensor = self.image_processor.preprocess(
                image, return_tensors="pt")["pixel_values"].half().to(
                self.model.device)
