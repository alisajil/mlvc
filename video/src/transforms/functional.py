...
def ycbcr420_to_444(y, uv, order=0, separate=False):
    ...
    """
    uv = np.asarray(scipy.ndimage.zoom(uv, (1, 2, 2), order=order))
    if separate:
        return y, uv
...
def ycbcr444_to_420(yuv, uv=None):
    """
    ...
    if uv is None:
        ...
        assert h % 2 == 0
        assert w % 2 == 0
        assert (1, h, w) == yuv.shape
        y = yuv

    # to 420
    uv = np.mean(np.reshape(uv, (2, h // 2, 2, w // 2, 2)), axis=(-1, -3))
...
def _check_input_tensor(tensor: Tensor) -> None:
    if (
        ...
        raise ValueError("Expected a 3D or 4D tensor with shape (Nx3xHxW) or (3xHxW) as input")
